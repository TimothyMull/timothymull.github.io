...
To shut down all ports you can use the interface range command to configure multiple interfaces at the same time. For example on SW1 the command would be `SW1(config)#int range e0/0-3,e1/0-3`.
Then to shut them down simple do `SW1(config-if-range)#shut`. This should be done on all switches.

All VTP domains must match for the VLAN DB to be propogated from the server to the clients. The command would be `SW1(config)#vtp domain TST`. This should be done on all switches.
